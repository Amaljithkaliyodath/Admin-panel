...
def dashboard(request):
    ...
    permission_count = sum(len(role.permissions) for role in Role.objects.all())
...
def users_view(request):
    ...
    sort_field = request.GET.get("sort_field", "name")  # Default sorting field
    ...
    if status_filter:
        users = users.filter(status=(status_filter == "1"))
...
def add_user(request):
    if request.method == 'POST':
        ...
        try:
            # Assuming you have a User model with email as unique
            role = Role.objects.get(id=role_id)
            user = User.objects.create(name=name, email=email, role=role, status=status)
